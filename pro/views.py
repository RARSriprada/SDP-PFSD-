...
def upi_payment(request):
    if request.method == 'POST':
        # Retrieve the UPI ID, phone number, and name from the form
        upi_id = request.POST.get('upi_id')
        phone_number = request.POST.get('phone_number')
        name = request.POST.get('name')

        # Here you can implement the logic to process the UPI payment
        # For demonstration purposes, let's just print the received data
        print(f"UPI ID: {upi_id}, Phone Number: {phone_number}, Name: {name}")

        # You can redirect the user to a success page or perform any other action
        return HttpResponse("Payment successful! Thank you.")

    # If the request method is GET, render the UPI payment page
...
def login(request):
    if request.method == 'POST':
        username = request.POST.get('username')  # Use get() method to retrieve form data
    ...
    if request.method == 'POST':
        ...
        if pass1 != pass2:
            ...
            if User.objects.filter(username=uname).exists():
                ...
                # Create the user using the default User model
                my_user = User.objects.create_user(username=uname, email=email, password=pass1)
                # Optionally, you can set additional attributes like first_name and last_name
...
def register(request):
    if request.method == 'POST':
        ...
        if pass1 != pass2:
            ...
            if custom.objects.filter(username=uname).exists():
                return HttpResponse("Username already exists. Please choose a different one.")
            elif custom.objects.filter(email=email).exists():
                return HttpResponse("Email already exists. Please use a different one.")
            else:
                # Create the custom user instance
                my_user = custom(username=uname, email=email)
...
def cart_view(request):
    ...
    # Check if payment mode is UPI
    payment_method = request.GET.get('payment_method')
    if payment_method == 'upi':
        # Redirect to 'upi.html' if payment mode is UPI
        return redirect('upi.html')
...
def about_us(request):
    # This view function renders the about us page
